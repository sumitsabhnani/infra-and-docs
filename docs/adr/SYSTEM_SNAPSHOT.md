...
- **Backfill Progress is Derived, Not Persisted** — async historical-price backfill jobs (`EodhdHistoricalPriceBackfillJob`, `CorporateActionSplitBackfillJob`) hold no progress state. The CSV path's `GET /api/v1/csv-import/backfill-status` and the per-portfolio path's `GET /api/v1/portfolio-sync-status` derive `{status, syncPhase, completedCount, totalCount, pendingListingIds}` directly from `market_data_price_daily` over the user's owned listings, with two pending-set filters: FAILED `ticker_resolution_queue` entries (joined via `holdings.acquired_listing_id`) and stale `MarketDataSymbol` rows whose `created_at` is older than **15 minutes** — the documented EODHD-has-given-up window (single-threaded executor at ~5 QPS clears even ~200 symbols in 2–3 min, so older-than-15-min-without-data is treated as EODHD-uncovered and silently dropped from `pendingListingIds`). The frontend renders at most one banner at a time with precedence SnapTrade > CSV > Prices; the SnapTrade "All data synced!" success copy is suppressed while prices are still SYNCING. The prices banner persists its poll start timestamp to `sessionStorage` so the 10-min stuck-state and 30-min hard-cap thresholds cannot be reset by reloading. No `*_backfill_status` table, no SSE/WS, no `BACKFILLING_PRICES` lifecycle on `SnaptradeConnection.sync_phase` (ADR-022, ADR-028).
